#!/usr/bin/env python3
"""
Standalone MCPToolService Test.

Tests MCPToolService by importing the module directly, bypassing __init__.py imports.
"""
...
# Import module directly to bypass __init__.py
spec = importlib.util.spec_from_file_location(
    "mcp_tool_service",
    project_root / "src" / "services" / "mcp_tool_service.py"
)
mcp_module = importlib.util.module_from_spec(spec)
spec.loader.exec_module(mcp_module)

# Extract what we need
MCPToolService = mcp_module.MCPToolService
CircuitBreaker = mcp_module.CircuitBreaker
CircuitBreakerState = mcp_module.CircuitBreakerState
...
║       (Standalone - bypasses autogen dependency)               ║
╚════════════════════════════════════════════════════════════════╝
""")
    ...
    try:
        ...
        print("\nNext Steps:")
        print("  1. Begin User Story 1: Adaptive Position Sizing")
        print("  2. Integration test with live agents via API")
        print("  3. Performance benchmarks (latency, cache hit rate)")
